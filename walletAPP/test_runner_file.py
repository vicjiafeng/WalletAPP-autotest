...
class TestSuite1(object):
    ...
    def test_brazil_login_request(self):
        with allure.step("1、登录界面输入账户密码"):
            print('输入登录手机号密码')
        with allure.step("2、点击登录按键"):
            login_response = brazil_login_request()
            print('调用登录接口，日志打印返回数据')
        with allure.step("3、执行断言"):
            try:
                assert 'success' == login_response['message']
                logger.info("巴西市场-测试用例：登录接口-断言成功，返回message为: {}".format(login_response['message']))
            except Exception as e:
                s = traceback.format_exc()
                logger.info("使用traceback输出异常: {}".format(s))
                logger.exception("巴西市场-测试用例：登录接口-断言失败，实际返回message：{}".format(login_response['message']))
                raise
    ...
    def test_history_request(self):
        ...
        with allure.step("3、执行断言"):
            try:
                ...
                logger.info("巴西市场-测试用例：账单列表接口-断言成功，返回message为: {}".format(history_response['message']))
            except Exception as e:
                s = traceback.format_exc()
                logger.info("使用traceback输出异常: {}".format(s))
                logger.exception("巴西市场-测试用例：账单列表接口-断言失败，实际返回message：{}".format(history_response['message']))
                raise
...
class TestSuite7(object):
    ...
    # 用例标题
    @allure.title('walletApp-Login')
    def test_mexico_login_request(self):
        with allure.step("1、登录界面输入账户密码"):
            print('输入登录手机号密码')
        with allure.step("2、点击登录按键"):
            login_response = mexico_login_request()
            print('调用登录接口，日志打印返回数据')
        with allure.step("3、执行断言"):
            try:
                assert 'success' == login_response['message']
                logger.info("墨西哥市场-测试用例：登录接口-断言成功，返回message为: {}".format(login_response['message']))
            except Exception as e:
                s = traceback.format_exc()
                logger.info("使用traceback输出异常: {}".format(s))
                logger.exception("墨西哥市场-测试用例：登录接口-断言失败，实际返回message：{}".format(login_response['message']))
                raise
...
class TestSuite10(object):
    ...
    # 用例标题
    @allure.title('walletApp-Transaction History List')
    def test_history_request_mexico(self):
        with allure.step("1、获取请求头Authorization"):
            print('获取请求头Authorization')
        with allure.step("2、进入History列表页"):
            history_response = TestCase02.history_request_mexico(self)
            print('调用账单列表接口,日志打印返回数据')
        with allure.step("3、执行断言"):
            try:
                assert 'success' == history_response['message']
                logger.info("墨西哥市场-测试用例：账单列表接口-断言成功，返回message为: {}".format(history_response['message']))
            except Exception as e:
                s = traceback.format_exc()
                logger.info("使用traceback输出异常: {}".format(s))
                logger.exception(
                    "墨西哥市场-测试用例：账单列表接口-断言失败，实际返回message：{}".format(history_response['message']))
                raise
